    ...
    <div class="paging">
      ...
      <c:if test="${totalCnt!=null && totalCnt!=0}">
        <c:if test="${ph.showPrev}">
          <a class="page" href="<c:url value="/product/goodslist${ph.sc.getQueryString(ph.beginPage-1)}"/>">&lt;</a>
        </c:if>
        <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
          <a class="page ${i==ph.sc.page? "paging-active" : ""}" href="<c:url value="/product/goodslist${ph.sc.getQueryString(i)}"/>">${i}</a>
        </c:forEach>
        <c:if test="${ph.showNext}">
          <a class="page" href="<c:url value="/product/goodslist${ph.sc.getQueryString(ph.endPage+1)}"/>">&gt;</a>
        </c:if>
      </c:if>
    </div>
  </div>
...
<script>
  let cd_name = ${param.cd_name};

  let showList = function(){
    $.ajax({
      type: 'GET',
      url: '/product/goodslist?cd_name='+cd_name,
      success : function(result){
        $("#content").html(toHtml(result));
      },
