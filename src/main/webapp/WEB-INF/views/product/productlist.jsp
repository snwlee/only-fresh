    ...
    <table border="1">
        <c:forEach var="ProductDto" items="${list}">
            <form action="/carts/${ProductDto.pdt_id}" id="form" method="post">
                <button class="cart_btn" type="submit">장바구니에 추가</button>
                <input type="number" value=1 name="pdt_qty">
                <p>태그 이름: ${ProductDto.tag_name}
                    제품명 :<a href="<c:url value="/product/read?pdt_id=${ProductDto.pdt_id}"/>"> <c:out
                            value="${ProductDto.title}"/></a>
